...
(function() {
    ...
    function switchSection(section) {
        ...
        if (sectionMap[section]) {
            ...
            if (section === 'visitors') {
                setTimeout(() => {
                    if (typeof initVisitorTracking === 'function') {
                        initVisitorTracking();
                    }
...
(function() {
    ...
    async function drawWorldMap(canvas, visitors) {
    ...
    function updateVisitorDisplay(data) {
        ...
        document.getElementById('uniqueLocations').textContent =
            new Set(data.visitors.map(v => v.country)).size;
        ...
        listEl.innerHTML = '';
        data.visitors.slice(-20).reverse().forEach(visitor => {
            const entry = document.createElement('div');
            entry.className = 'visitor-entry';

            const locationSpan = document.createElement('span');
            locationSpan.className = 'visitor-location';
            locationSpan.textContent = visitor.country;
        ...
        return new Promise((resolve) => {
            ...
            popup.innerHTML = `
                ...
                <p style="font-family: 'Courier New', monospace; color: #fff; font-size: 0.85rem; line-height: 1.6; margin-bottom: 1.5rem;">
                    Would you like to add your country to the visitor map? Only your country name will be displayed.
                </p>
    ...
    async function initVisitorTracking() {
        ...
        if (!consent) {
            document.getElementById('yourLocation').textContent = 'Not shared';
            return; // Don't track if user declined
        }

        // Get current location
        const location = await getVisitorLocation();
        document.getElementById('yourLocation').textContent = location.country;
    ...
    window.addEventListener('visitorsSectionShown', initVisitorTracking);
